...
def monte_carlo_simulation_with_tb(filename, n, n_samples, noise_std_x, noise_std_y, results_file, summary_file):
    ...
    dv1_ans, dv2_ans, sol_com, sol1, sol2, time_cost_nominal = tb.hohman_orbit3(x1, y1, r_aim, dv1_nominal)
    dv_nominal = np.linalg.norm(dv1_ans) + np.linalg.norm(dv2_ans)
    ...
    for i in range(n_samples):
        ...
        dv1_ans, dv2_ans, sol_com, sol1, sol2, time_cost_sample = tb.hohman_orbit3(x1, y1, r_aim, dv1)
        ...
        dv_outputs.append(dv_output)
        time_costs.append(time_cost_sample)

        # 各ループの入力値と出力値を保存
        input_output_data.append([noisy_dv1_x, noisy_dv1_y, dv_output, time_cost_sample])

    # 結果をCSVに保存
    results_df = pd.DataFrame(input_output_data, columns=["dv1_x", "dv1_y", "dv_output", "time_cost"])
    results_df.to_csv(results_file, index=False)

    # 出力の期待値（平均）と分散を計算
    dv_expected = np.mean(dv_outputs)
    dv_variance = np.var(dv_outputs)
    time_cost_expected = np.mean(time_costs)
    time_cost_variance = np.var(time_costs)
    ...
    summary_df = pd.DataFrame([summary_data], columns=[
        "n", "dv1_x", "dv1_y", "dv_nominal", "dv_expected", "dv_variance",
        "time_cost_nominal", "time_cost_expected", "time_cost_variance"
    ])
    ...
    return dv1_x, dv1_y, dv_nominal, dv_expected, dv_variance
...
if __name__ == '__main__':
    ...
    i = 492  # nの開始値
    j = 600  # nの終了値
    n_samples = 500  # モンテカルロシミュレーションのサンプル数
    ...
    summary_file = monte_carlo_dir / 'zmonte_carlo_summary_time.csv'  # サマリーファイル

    # nをiからjまで変化させてモンテカルロシミュレーションを実行
    for n in range(i, j + 1):
        results_file = monte_carlo_dir / f'zmonte_carlo_results_time_{n}.csv'  # 各nの結果ファイル
