...
def find_row_with_extreme_value(filename, column_name, rank=0, find_min=True, exclude_inf=False):
    with open(filename, mode='r') as file:
        csv_reader = csv.DictReader(file)
        rows = list(csv_reader)

        # Extract the target column values and optionally exclude inf values
        values = []
        for row in rows:
            value = float(row[column_name])
            if exclude_inf and math.isinf(value):
                continue
...
def main():
    ...
    while True:
        ...
        exclude_inf_str = input("infを除外しますか？ (yes/no): ")
        exclude_inf = exclude_inf_str.lower() == 'yes'

        try:
            result_row = find_row_with_extreme_value(filename, column_name, rank, find_min, exclude_inf)
